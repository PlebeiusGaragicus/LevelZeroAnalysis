...
# Step 1: Extract the earliest dispatch times for both AMR and PF&R for each 'incident'.
amr_data = data[data['dispatchStation'] == "AMR"].copy()
pfr_data = data[data['dispatchStation'].str.startswith("PF&R")].copy()

# Convert 'dispatch' columns to datetime
amr_data['dispatch'] = pd.to_datetime(amr_data['dispatch'])
pfr_data['dispatch'] = pd.to_datetime(pfr_data['dispatch'])

amr_min_dispatch = amr_data.groupby('incident')['dispatch'].min()
pfr_min_dispatch = pfr_data.groupby('incident')['dispatch'].min()

# Step 2: Merge these times with the merged_arrivals DataFrame.
merged_arrivals = pd.concat([pfr_min_arrival, amr_min_arrival, pfr_min_dispatch, amr_min_dispatch], 
                            axis=1, 
                            keys=['pfr_arrival', 'amr_arrival', 'pfr_dispatch', 'amr_dispatch'])

# Step 3: Filter the DataFrame based on the dispatch times.
filtered_merged_arrivals = merged_arrivals[merged_arrivals['amr_dispatch'] > merged_arrivals['pfr_dispatch']]

# Continue with the rest of the calculations
filtered_merged_arrivals.loc[:, 'wait_seconds'] = (filtered_merged_arrivals['amr_arrival'] - filtered_merged_arrivals['pfr_arrival']).dt.total_seconds()
filtered_merged_arrivals.loc[:, 'wait_seconds'] = filtered_merged_arrivals['wait_seconds'].apply(lambda x: max(0, x))


print(filtered_merged_arrivals.head(n=10))
export_filename = f"{PATH}/export- merged arrivals.csv"
print(">> exporting to: ", export_filename)
filtered_merged_arrivals.to_csv(export_filename)
